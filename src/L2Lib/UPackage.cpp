...
UPackage& UIndex::Serialize(class UPackage& Ar)
{
	...
	// Value записывается через деструктор UIndex
	Value = Index;
...
void UPackage::Open(char *fileName)
{
	...
	*((UPackage*)this) >> m_Header.ImportOffset;
	...
	{
		UExport t = Exports[i];
		while(t.iSuper > 0 && t.iSuper < (int32)Exports.Size())
		{
			if(Exports[t.iSuper].iSuper == t.iSuper) break;
			t = Exports[t.iSuper];
		}
	...
	{
		UImport t = Imports[i];
		while(t.Package < 0 && (-t.Package - 1) < (int32)Imports.Size())
		{
			if(Imports[-t.Package - 1].Package == t.Package) break;
	...
	for(uint32 i = 0; i < Exports.Size(); i++)
	{
		...
		if(UTIL_strcmpi(Exports[i].Name, name) == 0 && UTIL_strcmpi(Exports[i].BaseClass, "Package") != 0)
		{
			...
			*((UPackage*)this) >> UIndex(ind);
			...
			fprintf(stderr, "GetUObject result=%p\n", (void*)Result); fflush(stderr); fclose(_f);
...
UPackage& UPackage::operator >> (class UObject*& Res)
{
	...
	*((UPackage*)this) >> UIndex(Index);
	...
	else
	{
		...
		{
			const char* bc = Exports[Index - 1].BaseClass;
			if (bc && (
				UTIL_strcmpi(bc,"Model")==0 ||
				UTIL_strcmpi(bc,"Shader")==0 ||
				UTIL_strcmpi(bc,"FinalBlend")==0 ||
				UTIL_strcmpi(bc,"Combiner")==0 ||
				UTIL_strcmpi(bc,"TexModifier")==0 ||
				UTIL_strcmpi(bc,"Texture")==0
			))

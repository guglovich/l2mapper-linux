...
{
public:
	int32* RefPtr;   // указатель вместо reference — позволяет копировать/перемещать
	int32  Value;    // буфер для временных объектов
public:
	UPackage& Serialize(UPackage& Ar);
	IMPLEMENT_USERIALIZER(UIndex);
public:
	// Конструктор из lvalue — запишет результат обратно
	UIndex(int32& idx) : RefPtr(&idx), Value(idx) {}
	// Конструктор из rvalue int (временный — результат через Value)
	UIndex() : RefPtr(nullptr), Value(0) {}
	~UIndex() { if (RefPtr) *RefPtr = Value; }
};

...
void UTerrainSector::Init() // int32 InSectorX, int32 InSectorZ
{
	...
	for(uint16 iy = 0; iy < 17; iy++)
	{
		for(uint16 ix = 0; ix < 17; ix++)
		{
			...
			//m_VertexArray[(iy * 17 + ix) * 3 + 1] = *(uint16*)(m_TerrainInfo->TerrainMap->Data + (min<uint16>(hmy, 255) * m_TerrainInfo->TerrainMap->Width + min<uint16>(hmx, 255)) * 2) / 256. * m_TerrainInfo->TerrainScale.Z - Restit; //  + m_TerrainInfo->m_Location.Y / m_TerrainInfo->m_DrawScale
			float TerrainHeight = *((uint16*)m_TerrainInfo->TerrainMap->Data + (min<uint16>(hmy, 255) * m_TerrainInfo->TerrainMap->Width + min<uint16>(hmx, 255)));

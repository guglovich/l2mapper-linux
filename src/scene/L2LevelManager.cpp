...
void L2LevelManager::loadLevel(int map_x, int map_y)
{
	...
	if (level) level->setL2LibVars(varsHolder);
...
void* L2LevelManager::loadTileThread(void *arg)
{
...
{
	loadingTilesCounter += tiles->Size();
	for(uint16 i = 0; i < tiles->Size(); i++)
	{
		loadTileThread((*tiles)[i]);
	}
}

void L2LevelManager::resetCamera()
{
	if (m_Maps.Size() > 0 && m_Maps[0]) {
		ULevel* level = m_Maps[0];
		g_camera.setTarget(
			level->_mapLoc.x + MAP_SIZE_X / 2,
			-4500,
			level->_mapLoc.z + MAP_SIZE_Z / 2);
	}
}
	...
	for(uint32 l = 0; l < m_Maps.Size(); l++)
	{
		...
		glPushMatrix();
			for(uint32 i = 0; i < level->Model->Nodes.Size(); i++)
			{
				...
				if(!isWater && (aPos - g_camera.getPos()).Length() > g_scene.getVisibleDist()) continue;
				...
				{ static int rn=0; if(rn<5){fprintf(stderr,"Node nv=%d\n",Node.NumVertices);fflush(stderr);rn++;} }
					g_shader.Disable();
			glDisable(GL_TEXTURE_2D);
